...
//$consulta =mysql_query("SELECT nombre, empresa, puesto,telefono, email, conferencia FROM `registro_conferencias` inner join regconf_tipos on registro_conferencias.idreg=regconf_tipos.idreg  ");
 ...
 $conexion = new mysqli("expocomercio.db.11354178.hostedresource.com","expocomercio","Expo14!sar",'expocomercio',3306);
	...
	$consulta = "select * from prerregistro order by idprerregistro";
	...
	if($resultado->num_rows > 0 ){
		...
		$objPHPExcel->getProperties()->setCreator("Epoferia del Comercio") //Autor
							 ...
							 ->setDescription("Reporte de pre egistros")
							 ->setKeywords("reporte pre registros")
							 ->setCategory("Reporte excel");

		$tituloReporte = "LISTA DE PRE REGISTROS";
		$titulosColumnas = array("NOMBRE(S)","APELLIDO(S)","COLONIA", "CIUDAD", "ESTADO","PAIS", "EMAIL","CELULAR");
		...
		while ($fila = $resultado->fetch_array()) {
			$objPHPExcel->setActiveSheetIndex(0)
        		    ->setCellValue('A'.$i,  utf8_decode($fila['nombres']))
		            ->setCellValue('B'.$i,  utf8_decode($fila['apellidopaterno']))
        		    ->setCellValue('C'.$i,  utf8_decode($fila['colonia']))
            		->setCellValue('D'.$i, $fila['ciudad'])
					->setCellValue('E'.$i, $fila['estado'])
					->setCellValue('F'.$i,utf8_decode($fila['pais']))
					
					->setCellValue('G'.$i,utf8_decode($fila['email']))
					->setCellValue('H'.$i,utf8_decode($fila['celular']));
					$i++;
		}
		...
		$objPHPExcel->getActiveSheet()->setTitle('Pre registros');
